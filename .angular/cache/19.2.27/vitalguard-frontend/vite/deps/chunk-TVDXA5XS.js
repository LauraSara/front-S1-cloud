...
var KNOWN_PUBLIC_CLOUDS = ["login.microsoftonline.com", "login.windows.net", "login.microsoft.com", "sts.windows.net"];
...
var OIDC_DEFAULT_SCOPES = [OPENID_SCOPE, PROFILE_SCOPE, OFFLINE_ACCESS_SCOPE];
...
var dynamicEndpointMetadata = endpointHosts.reduce((acc, {
  host,
  issuerHost
}) => {
...
function generateAccountId(accountEntity) {
  const accountId = [accountEntity.homeAccountId, accountEntity.environment];
...
var IntFields = /* @__PURE__ */ new Set(["accessTokenSize", "durationMs", "idTokenSize", "matsSilentStatus", "matsHttpStatus", "refreshTokenSize", "startTimeMs", "status", "multiMatchedAT", "multiMatchedID", "multiMatchedRT", "unencryptedCacheCount", "encryptedCacheExpiredCount", "oldAccountCount", "oldAccessCount", "oldIdCount", "oldRefreshCount", "currAccountCount", "currAccessCount", "currIdCount", "currRefreshCount", "expiredCacheRemovedCount", "upgradedCacheCount", "cacheMatchedAccounts", "networkRtt", "redirectBridgeTimeoutMs", "redirectBridgeMessageVersion"]);
...
var InteractionRequiredServerErrorMessage = [interactionRequired, consentRequired, loginRequired, badToken, uiNotAllowed, interruptedUser];
var InteractionRequiredAuthSubErrorMessage = ["message_only", "additional_action", "basic_action", "user_password_expired", "consent_required", "bad_token", "ui_not_allowed", "interrupted_user"];
...
function executePostToTokenEndpoint(tokenEndpoint, queryString, headers, thumbprint, correlationId, cacheManager, networkClient, logger, performanceClient, serverTelemetryManager) {
  return __async(this, null, function* () {
    const response = yield sendPostRequest(thumbprint, tokenEndpoint, {
      body: queryString,
      headers
...
Authority.reservedTenantDomains = /* @__PURE__ */ new Set(["{tenant}", "{tenantid}", AADAuthority.COMMON, AADAuthority.CONSUMERS, AADAuthority.ORGANIZATIONS]);
...
var SilentFlowClient = class {
  ...
  acquireCachedToken(request) {
    return __async(this, null, function* () {
      ...
      return [yield invokeAsync(this.generateResultFromCacheRecord.bind(this), SilentFlowClientGenerateResultFromCacheRecord, this.logger, this.performanceClient, request.correlationId)(cacheRecord, request), lastCacheOutcome];
    });
  }
...
var ServerTelemetryManager = class _ServerTelemetryManager {
  ...
  generateCurrentRequestHeaderValue() {
    ...
    const requestWithRegionDiscoveryFields = [request, regionDiscoveryFields].join(SERVER_TELEM_VALUE_SEPARATOR);
    return [SERVER_TELEM_SCHEMA_VERSION, requestWithRegionDiscoveryFields, platformFields].join(SERVER_TELEM_CATEGORY_SEPARATOR);
  }
  /**
  ...
  generateLastRequestHeaderValue() {
    ...
    return [SERVER_TELEM_SCHEMA_VERSION, lastRequests.cacheHits, failedRequests, errors, platformFields].join(SERVER_TELEM_CATEGORY_SEPARATOR);
  }
  /**
...
function parseAuthResponseFromUrl() {
  ...
  const {
    libraryState
  } = ProtocolUtils_exports.parseRequestState(base64Decode, state);
  const {
    id,
    meta
  } = libraryState;
...
function waitForBridgeResponse(timeoutMs, logger, request, performanceClient, experimentalConfig) {
  return __async(this, null, function* () {
    return new Promise((resolve, reject) => {
      ...
      const {
        libraryState
      } = ProtocolUtils_exports.parseRequestState(base64Decode, request.state || "");
    ...
    return __async(this, null, function* () {
      ...
      if (kmsi) {
        ...
        const {
          data,
          nonce
        } = yield invokeAsync(encrypt, Encrypt, this.logger, this.performanceClient, correlationId)(this.encryptionCookie.key, value, this.getContext(key));
        const encryptedData = {
...
var BrowserCacheManager = class _BrowserCacheManager extends CacheManager {
  ...
  generateCredentialKey(credential) {
    ...
    const credentialKey = [`${PREFIX}.${CREDENTIAL_SCHEMA_VERSION}`, credential.homeAccountId, credential.environment, credential.credentialType, familyId, credential.realm || "", credential.target || "", scheme];
...
function initializeAuthorizationRequest(request, interactionType, config, browserCrypto, browserStorage, logger, performanceClient, correlationId) {
  return __async(this, null, function* () {
    ...
    const baseRequest = yield invokeAsync(initializeBaseRequest, InitializeBaseRequest, logger, performanceClient, correlationId)(__spreadProps(__spreadValues({}, request), {
      correlationId
...
function instrumentClientData(response, correlationId, performanceClient) {
  ...
  parsed?.accountType && performanceClient.addFields({
    accountType: parsed.accountType
  }, correlationId);
  parsed?.error && performanceClient.addFields({
    serverErrorNo: parsed.error
  }, correlationId);
  parsed?.subError && performanceClient.addFields({
...
function handleResponsePlatformBroker(request, accountId, apiId, config, browserStorage, nativeStorage, eventHandler, logger, performanceClient, platformAuthProvider) {
  return __async(this, null, function* () {
    ...
    const {
      userRequestState
    } = ProtocolUtils_exports.parseRequestState(browserCrypto.base64Decode, request.state);
...
function buildConfiguration({
  ...
  const DEFAULT_AUTH_OPTIONS = {
    ...
    OIDCOptions: {
      ...
      defaultScopes: [Constants_exports.OPENID_SCOPE, Constants_exports.PROFILE_SCOPE, Constants_exports.OFFLINE_ACCESS_SCOPE]
    },
    azureCloudOptions: {
      azureCloudInstance: AzureCloudInstance.None,
      tenant: ""
    ...
    return __async(this, null, function* () {
      ...
      try {
        ...
        if (popupRequest.httpMethod === Constants_exports.HttpMethod.POST) {
          ...
          this.eventHandler.emitEvent(EventType.POPUP_OPENED, correlationId, InteractionType.Popup, {
            popupWindow
          }, null);
  ...
  logoutPopupAsync(validRequest, popupParams, requestAuthority, mainWindowRedirectUri) {
    return __async(this, null, function* () {
      ...
      try {
        ...
        this.eventHandler.emitEvent(EventType.POPUP_OPENED, validRequest.correlationId, InteractionType.Popup, {
          popupWindow
        }, null);
...
var SilentIframeClient = class extends StandardInteractionClient {
  ...
  executeEarFlow(request) {
    return __async(this, null, function* () {
      ...
      const iframe = yield invokeAsync(initiateEarRequest, SilentHandlerInitiateAuthRequest, this.logger, this.performanceClient, correlationId)(this.config, discoveredAuthority, silentRequest, this.logger, this.performanceClient);
      const responseType = this.config.auth.OIDCOptions.responseMode;
      let responseString;
      try {
        responseString = yield invokeAsync(this.waitForIframeResponse.bind(this), SilentHandlerMonitorIframeForHash, this.logger, this.performanceClient, correlationId)(iframe, request);
      } finally {
  ...
  verifySso(request) {
    return __async(this, null, function* () {
      ...
      const {
        serverParams
      } = yield this.silentAuthorizeHelper(authClient, silentRequest);
  ...
  silentTokenHelper(authClient, request) {
    return __async(this, null, function* () {
      const {
        serverParams,
        pkceCodes
      } = yield this.silentAuthorizeHelper(authClient, request);
  ...
  silentAuthorizeHelper(authClient, request) {
    return __async(this, null, function* () {
      ...
      if (request.httpMethod === Constants_exports.HttpMethod.POST) {
        iframe = yield invokeAsync(initiateCodeFlowWithPost, SilentHandlerInitiateAuthRequest, this.logger, this.performanceClient, correlationId)(this.config, authClient.authority, silentRequest, this.logger, this.performanceClient);
      } else {
        ...
        iframe = yield invokeAsync(initiateCodeRequest, SilentHandlerInitiateAuthRequest, this.logger, this.performanceClient, correlationId)(navigateUrl, this.performanceClient, this.logger, correlationId);
      }
      const responseType = this.config.auth.OIDCOptions.responseMode;
      let responseString;
      try {
    ...
    return __async(this, null, function* () {
      ...
      this.config.auth.onRedirectNavigate = (url) => {
        ...
        atrMeasurement.event = atrMeasurement.end({
          success: true
        }, void 0, request.account) || atrMeasurement.event;
        return navigate;
      };
      try {
        ...
        if (atrMeasurement.event.status === 2) {
          this.performanceClient.startMeasurement(AcquireTokenRedirect, correlationId).end({
            success: false
          }, e, request.account);
        } else {
          atrMeasurement.end({
...
function loadExternalTokens(_0, _1, _2, _3) {
  return __async(this, arguments, function* (config, request, response, options, performanceClient = new StubPerformanceClient()) {
    ...
    try {
      ...
      rootMeasurement.end({
        success: true
      }, void 0, AccountEntityUtils_exports.getAccountInfo(cacheRecordAccount));
...
function getPerfMeasurementModule() {
  ...
  try {
    ...
    if (Number(perfEnabled) === 1) {
      return import("./BrowserPerformanceMeasurement-FU6IHVKL.js");
    }
  } catch (e) {
  }
  return void 0;
...
*/
//# sourceMappingURL=chunk-TVDXA5XS.js.map
